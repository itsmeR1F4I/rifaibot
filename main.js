require('./config.js')
let { WAConnection: _WAConnection, WA_MESSAGE_STUB_TYPES, Presence } = require('@adiwajshing/baileys')
...
conn.browserDescription = ['Rifaigamteng :V', 'python', 'FAIII']
...
if (!opts['test']) setInterval(() => {
  conn.logger.info('LAGI NGESAVE DATABASE NIH')
  if (JSON.stringify(global.DATABASE.data) == lastJSON) conn.logger.info('DATABASE NYA DAH SAMA TOD')
  else {
    global.DATABASE.save()
    conn.logger.info('YEY DAH KESAVE')
